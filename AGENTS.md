...
- For new entries in `_publications/`, always set `publication_type` in front matter. Use `peer_reviewed` for published peer-reviewed work and `working_paper` for working papers. The publications page renders `Peer-Reviewed` first and `Working Papers` second, with newest years first inside each section.
